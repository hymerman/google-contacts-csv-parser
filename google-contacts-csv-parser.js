...
function writeFilePromise(file_name, file_contents) {
  ...
  winston.info("Write requested");

  fs.writeFile(file_name, file_contents, {encoding:"utf-8"}, function (error) {
    if(error) {
      winston.error("Write error");
      deferred.reject(new Error(error));
    } else {
      winston.info("Write success");
      deferred.resolve();
    }
  });
    ...
    for(var contact_index in data) {
      ...
      for(var property_name in contact) {
        if(contact[property_name] === null || contact[property_name] === undefined || contact[property_name] == "") {
          ...
          winston.debug("Checking whether property should be an array: " + property_name);
          for(var property_which_should_be_array_index in properties_which_should_be_arrays) {
            ...
            winston.debug("Checking " + property_which_should_be_array);
            if(property_name.indexOf(property_which_should_be_array) === 0) {
              winston.debug("Found field which should be an array: " + property_name);
              ...
              winston.debug("Extracted array index: " + array_index + ", property name: " + sub_object_property_name);
              if(contact[property_which_should_be_array] === undefined) {
                // Array doesn't exist yet. Create it!
                winston.debug("Array doesn't exist yet. Creating it.");
                ...
                winston.debug("Array exists but we've come to a new item - appending it to the array.");
                ...
                winston.debug("Array exists, and object at this index exists; adding data to it.");
              }
      ...
      for(var outer_property in properties_objects_in_array_properties_should_have)
      {
        ...
        winston.debug("Found contact with field to check: " + outer_property + "(length:" + contact[outer_property].length + ") each must have " + inner_property);

        for(var array_index = contact[outer_property].length - 1; array_index >= 0; --array_index) {
          winston.debug("Checking whether " + JSON.stringify(contact[outer_property][array_index]) + " has property " + inner_property);
          if(contact[outer_property][array_index][inner_property] === null || contact[outer_property][array_index][inner_property] === undefined || contact[outer_property][array_index][inner_property] == "") {
            winston.debug("Doesn't have property; deleting");
            contact[outer_property].splice(array_index, 1);
          }
        }

        if(contact[outer_property].length == 0) {
          winston.debug("No more values exist in array; deleting whole property.");
